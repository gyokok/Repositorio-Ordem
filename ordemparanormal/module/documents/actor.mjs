...
export class OrdemActor extends Actor {
	...
	prepareBaseData() {
        ...
        // --- ADICIONADO: Calcular perícias para Ameaças também ---
	...
	async _prepareBaseSkills(system) {
		...
		for (const [keySkill, skill] of Object.entries(skills)) {
			...
			if (skill && Array.isArray(skill.attr) && skill.attr.length > 0) {
                ...
                // CORREÇÃO: Se for a perícia livre e tiver nome, usa ele.
	...
	getRollData() {
        ...
        // --- ATUALIZADO: Verifica se a perícia de iniciativa existe (vale para Agente e Ameaça agora) ---
		if (system.skills?.initiative) {
            ...
            // Se for ameaça e tiver value direto na skill (caso use estrutura antiga), soma também, 
            // mas com a nova estrutura 'degree.value' é o que conta.
            const flatValue = Number(system.skills.initiative.value) || 0;
            
            // Prioriza o grau calculado, mas se for 0 e tiver valor fixo, usa o fixo (segurança)
